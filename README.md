...
为什么关于数据库操作，推荐使用 基本操作，而不是直接操作数据库，使用到数据库一些复杂的 api 实现呢？。**性能** 和 **高复用**
获取信息?

- 前端一体化开发
  - 用工程化思想，解决前端和后端框架一体的问题

## 致敬

20211028 在 egg 文档看到思路相同的做法。  
20211029 看到 字节 web infra 团队的 Modern.js 。大喜。这就是我要做的内容。
...
这两者鲜明的代表分别是 **Restfull Api** 和 **GraphQL**。

**Restfull Api**根据资源定义路由接口，能够实现资源获取的的最小单元。
